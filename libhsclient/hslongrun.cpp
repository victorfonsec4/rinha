...
#include "libhsclient/auto_ptrcontainer.hpp"
#include "libhsclient/hstcpcli.hpp"
#include "libhsclient/mutex.hpp"
...
struct auto_mysql_res : private noncopyable {
  auto_mysql_res(MYSQL *db) { res = mysql_store_result(db); }
  ~auto_mysql_res() {
    if (res) {
...
struct auto_mysql_stmt : private noncopyable {
  auto_mysql_stmt(MYSQL *db) { stmt = mysql_stmt_init(db); }
  ~auto_mysql_stmt() {
    if (stmt) {
...
struct hs_longrun_stat {
  ...
  hs_longrun_stat()
      : verify_error_count(0), runtime_error_count(0), unknown_count(0),
        success_count(0) {}
...
struct hs_longrun_thread_base : public thread_base {
  ...
  unsigned int seed;
  hs_longrun_thread_base(const arg_type &arg) : arg(arg), seed(0) {
  ...
  int verify_update(const std::string &k, const std::string &v1,
                    ...
                    record_value &rec, uint32_t num_rows,
                    bool cur_unknown_state);
...
int hs_longrun_thread_base::verify_update(const std::string &k,
                                          const std::string &v1,
                                          const std::string &v2,
                                          const std::string &v3,
                                          record_value &rec, uint32_t num_rows,
...
int hs_longrun_thread_base::verify_read(const std::string &k, uint32_t num_rows,
                                        uint32_t num_flds,
                                        const std::string rrec[4],
                                        record_value &rec) {
...
int hs_longrun_thread_base::verify_readnolock(const std::string &k,
                                              uint32_t num_rows,
                                              uint32_t num_flds,
                                              const std::string rrec[4]) {
  int ret = 0;
...
struct hs_longrun_thread_hs : public hs_longrun_thread_base {
  hs_longrun_thread_hs(const arg_type &arg) : hs_longrun_thread_base(arg) {}
  void run();
...
std::string to_string(const string_ref &s) {
  return std::string(s.begin(), s.size());
}

void hs_longrun_thread_hs::run() {
...
int hs_longrun_thread_hs::op_insert(record_value &rec) {
  ...
  const string_ref op_args[4] = {to_string_ref(k), to_string_ref(v1),
                                 to_string_ref(v2), to_string_ref(v3)};
  cli->request_buf_exec_generic(0, op_ref, op_args, 4, 1, 0, string_ref(), 0, 0,
                                0, 0);
  cli->request_send();
  if (check_hs_error("op_insert_send", &rec) != 0) {
    return 1;
  }
  size_t numflds = 0;
...
int hs_longrun_thread_hs::op_delete(record_value &rec) {
  const std::string k = rec.key;
  const string_ref op_ref("=", 1);
  ...
  cli->request_buf_exec_generic(0, op_ref, op_args, 1, 1, 0, modop_ref, 0, 0, 0,
                                0);
  cli->request_send();
  if (check_hs_error("op_delete_send", &rec) != 0) {
    return 1;
  }
  size_t numflds = 0;
  cli->response_recv(numflds);
  if (check_hs_error("op_delete_recv", &rec) != 0) {
    return 1;
  }
  const string_ref *row = cli->get_next_row();
  const bool op_success = (numflds > 0 && row != 0 && to_string(row[0]) == "1");
  int ret = 0;
  if (!rec.unknown_state) {
    if (!rec.deleted && !op_success) {
      ++stat.verify_error_count;
...
int hs_longrun_thread_hs::op_update(record_value &rec) {
  const std::string k = rec.key;
  ...
  const string_ref modop_args[4] = {to_string_ref(k), to_string_ref(v1),
                                    to_string_ref(v2), to_string_ref(v3)};
  cli->request_buf_exec_generic(0, op_ref, op_args, 1, 1, 0, modop_ref,
                                modop_args, 4, 0, 0);
  cli->request_send();
  if (check_hs_error("op_update_send", &rec) != 0) {
    return 1;
  }
  size_t numflds = 0;
  cli->response_recv(numflds);
  if (check_hs_error("op_update_recv", &rec) != 0) {
    return 1;
  }
  const string_ref *row = cli->get_next_row();
  uint32_t num_rows =
      row ? atoi_uint32_nocheck(row[0].begin(), row[0].end()) : 0;
  cli->response_buf_remove();
  const bool cur_unknown_state = (num_rows == 1);
  return verify_update(k, v1, v2, v3, rec, num_rows, cur_unknown_state);
}

int hs_longrun_thread_hs::op_read(record_value &rec) {
  const std::string k = rec.key;
  const string_ref op_ref("=", 1);
  const string_ref op_args[1] = {
      to_string_ref(k),
  };
  cli->request_buf_exec_generic(0, op_ref, op_args, 1, 1, 0, string_ref(), 0, 0,
                                0, 0);
  cli->request_send();
  if (check_hs_error("op_read_send", 0) != 0) {
    return 1;
  }
  size_t num_flds = 0;
  size_t num_rows = 0;
  cli->response_recv(num_flds);
  if (check_hs_error("op_read_recv", 0) != 0) {
    return 1;
  }
  const string_ref *row = cli->get_next_row();
  std::string rrec[4];
...
int hs_longrun_thread_hs::op_readnolock(int key) {
...
int hs_longrun_thread_hs::check_hs_error(const char *mess, record_value *rec) {
  ...
  while (arg.sh.running) {
    if (!connected) {
      if (!mysql_real_connect(db, mysql_host.c_str(), mysql_user.c_str(),
                              mysql_passwd.c_str(), mysql_dbname.c_str(),
                              mysql_port, 0, 0)) {
...
int hs_longrun_thread_my::op_delins(record_value &rec) {
  const std::string k = rec.key;
  ...
  do {
    ...
    snprintf(query, 1024, "delete from hstesttbl where k = '%s'", k.c_str());
    ...
    snprintf(query, 1024,
             "insert into hstesttbl values ('%s', '%s', '%s', '%s')", k.c_str(),
             v1.c_str(), v2.c_str(), v3.c_str());
    if (mysql_query(db, query) != 0) {
      if (arg.sh.verbose >= 20) {
        fprintf(stderr, "mysql: e=[%s] q=[%s]\n", mysql_error(db), query);
...
int hs_longrun_thread_my::op_read(record_value &rec) {
  const std::string k = rec.key;
  char query[1024] = {0};
  const int len = snprintf(
      query, 1024, "select k,v1,v2,v3 from hstesttbl where k='%s'", k.c_str());
...
void hs_longrun_thread_my::show_mysql_error(const char *mess,
                                            record_value *rec) {
  ++stat.runtime_error_count;
  if (arg.sh.verbose > 0) {
...
void mysql_do(MYSQL *db, const char *query) {
...
void hs_longrun_init_table(const config &conf, int num_prepare,
  ...
  mysql_do(db, "create table hstesttbl ("
               "k int primary key,"
  ...
  for (int i = 0; i < num_prepare; ++i) {
    ...
    char buf[1024];
    snprintf(buf, 1024,
             "insert into hstesttbl(k, v1, v2, v3) values"
             "(%d, '%s', '%s', '%s')",
             i, v1.c_str(), v2.c_str(), v3.c_str());
    mysql_do(db, buf);
...
int hs_longrun_main(int argc, char **argv) {
  hs_longrun_shared shared;
  ...
  hs_longrun_init_table(shared.conf, always_filled ? table_size : 0, shared);
...
int main(int argc, char **argv) { return dena::hs_longrun_main(argc, argv); }
