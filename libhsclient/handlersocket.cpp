...
struct HandlerSocket {
  ...
  static VALUE initialize(int argc, VALUE *argv, VALUE self) {
    ...
    rb_scan_args(argc, argv, "05", &host, &port, &timeout, &listen_backlog,
                 &verbose_level);
    ...
    if (NIL_P(listen_backlog)) {
      ...
      listen_backlog =
          rb_check_convert_type(listen_backlog, T_STRING, "String", "to_s");
  ...
  static VALUE open_index(VALUE self, VALUE id, VALUE db, VALUE table,
                          VALUE index, VALUE fields) {
    HandlerSocket *p;
    ...
    do {
      p->tcpcli->request_buf_open_index(FIX2INT(id), RSTRING_PTR(db),
                                        RSTRING_PTR(table), RSTRING_PTR(index),
                                        RSTRING_PTR(fields));
  ...
  static VALUE execute_single(int argc, VALUE *argv, VALUE self) {
    ...
    rb_scan_args(argc, argv, "34", &id, &op, &keys, &limit, &skip, &modo,
                 &modvals);
  ...
  static VALUE execute_update(VALUE self, VALUE id, VALUE op, VALUE keys,
                              VALUE limit, VALUE skip, VALUE modvals) {
    return execute_internal(self, id, op, keys, limit, skip, "U", modvals);
  }

  static VALUE execute_delete(VALUE self, VALUE id, VALUE op, VALUE keys,
                              VALUE limit, VALUE skip) {
  ...
  static VALUE execute_insert(VALUE self, VALUE id, VALUE fvals) {
    ...
    char *modop = NULL;
    return execute_internal(self, id, op, fvals, INT2FIX(0), INT2FIX(0), modop,
                            Qnil);
  }
  ...
  static void init() {
    ...
    rb_define_method(rb_cHandlerSocket, "get_error_code", __F(&get_error_code),
                     0);
    ...
    rb_define_method(rb_cHandlerSocket, "execute_single", __F(&execute_single),
                     -1);
    rb_define_method(rb_cHandlerSocket, "execute_find", __F(&execute_single),
                     -1);
    rb_define_method(rb_cHandlerSocket, "execute_multi", __F(&execute_multi),
                     1);
    rb_define_method(rb_cHandlerSocket, "execute_update", __F(&execute_update),
                     6);
    rb_define_method(rb_cHandlerSocket, "execute_delete", __F(&execute_delete),
                     5);
    rb_define_method(rb_cHandlerSocket, "execute_insert", __F(&execute_insert),
                     2);
  }

private:
  static VALUE execute_internal(VALUE self, VALUE v_id, VALUE v_op,
                                VALUE v_keys, VALUE v_limit, VALUE v_skip,
                                char *modop, VALUE v_modvals) {
    HandlerSocket *p;
    VALUE retval = Qnil;
    ...
    do {
      ...
      int id = FIX2INT(v_id);
      dena::string_ref op =
          dena::string_ref(RSTRING_PTR(v_op), RSTRING_LEN(v_op));
      ...
      tcpcli->request_buf_exec_generic(id, op, &keyarr[0], keyarr.size(), limit,
                                       skip, modop_ref, &mvarr[0],
                                       mvarr.size());
...
void Init_handlersocket() { HandlerSocket::init(); }
