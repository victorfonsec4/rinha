...
#define FD_SETSIZE                                                             \
  1024 /* Max number of file descriptor bits in                                \
...
#define HOT_METHOD __attribute__((section("hot_code_section")))
#else
#define HOT_METHOD
#endif
...
#if (__GNUC__ > 2) || (__GNUC__ == 2 && __GNUC_MINOR > 94)
#define HOT_DATA __attribute__((section("hot_data_section")))
#else
#define HOT_DATA
#endif
...
static inline double rint(double x) {
  ...
  */
  if (x > 0.0)
    i += (double)((f > 0.5) || (f == 0.5 && i <= (double)MAX_EXACT_INTEGER &&
                                (longlong)i % 2));
  else
    i -= (double)((f < -0.5) || (f == -0.5 && i >= (double)-MAX_EXACT_INTEGER &&
                                 (longlong)i % 2));
