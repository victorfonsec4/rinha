...
std::string to_json(const Transaction &transaction) {
  return absl::StrCat(kTransacaoValor, transaction.value, kTransacaoTipo,
                      transaction.value > 0 ? "c" : "d", kTransacaoDescricao,
                      transaction.description, kTransacaoRealizada,
                      transaction.timestamp, kTransacaoEnd);
}
